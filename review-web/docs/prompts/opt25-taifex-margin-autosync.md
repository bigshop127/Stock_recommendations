...
- ⚠️ **`stale` 旗標要跟著進記憶體快取**，而且 `cached` / `stale` 不要落地寫進磁碟檔。第一版踩過這個坑：`serveDisk()` 把磁碟那份（裡面寫死了 `stale:false`）塞進記憶體快取，結果一次抓取失敗之後的六小時內，每個命中都回 `stale:false`，把舊保證金講成當前值。修法＝標記後才進快取、命中時把旗標帶出去、磁碟只存資料本身。
- ⚠️ stale 狀態下的 TTL 要縮短（實作用 15 分鐘）。用同一個 6 小時 TTL 的話，期交所一次暫時性失敗會讓我們半天不再去問。
...
去重：把「已通知過的保證金數值」記進 `data/futures_alert_state.json`（`last_margin_notice_key`），同一組數值只講一次。

> ⚠️ **不要用 `Date` 當去重鍵**——它每個交易日都會變（見 §7 的實測結論），用它去重等於沒去重。
...
- ✅ **`Date` 欄位語意已確認（2026-07-30 實測）**：它是**資料日**，每個交易日都推進一天，不是「公告日」也不是「生效日」。證據：7/29 抓到 `2026-07-28`，7/30 抓到 `2026-07-29`，期間 TX/MTX/TMF 三個數字完全沒變。期交所每個交易日重發一份現行值。

  兩個後果：
  1. **UI 文案不能寫「公告」**——會讓人以為當天有調整。已全部改成「期交所 YYYY-MM-DD 現行值」。
  2. **§4 的去重鍵不能用 `Date`**。原本寫的 `last_margin_notice_date` 在這個語意下永遠去重不掉：只要設定沒同步，每個交易日的 `Date` 都是新的，就會每天寄一封一模一樣的信。已改成用**保證金數值**當鍵（`last_margin_notice_key = "159000/122000"`），同一組數值只講一次。
- 📌 若哪天發現 `Date` 與期貨商通知的生效日有落差，仍以期貨商通知為準。
