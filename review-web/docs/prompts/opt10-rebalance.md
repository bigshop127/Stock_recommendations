...
> **2026-07-07 增修 I（Claude 直接實作）＝防守端改「固定 10 萬現金＋債券池 6:4」**：使用者定案——閒置資金改為**固定保留 100,000 現金**＋剩餘防守資金依 **00687B（國泰20年美債）:00953B（群益優選非投等債）＝6:4** 分配，且兩檔債券 ETF 要跟 00631L 一樣顯示應買進/賣出金額、股數、現價、抓最新價。**β 模型**：現金與債券市值皆視為 β=0（簡化，忽略債券自身波動），β 分母＝00631L 市值＋防守端（現金＋債券市值）。**多資產報價單**：`Trade.code`（缺省＝00631L 相容舊資料）、`aggregatePortfolio` 全域按日累算＋**共用現金池**（任一標的買扣賣加——因增修H 現金是衍生值，買債必須經報價單才會扣現金）。`cash_reserve`/`bond_split` 可在進階設定調整。gateway/告警腳本/範本 json 同步（告警腳本抓 3 檔收盤價、債券抓不到退用同步價；舊持倉檔無 bonds 行為不變）。詳見 ROADMAP §8 opt10 增修 I 紀錄。
