...
| 10 | 「正2 + 現金」Beta 再平衡系統（使用者個人投組工具） | `opt10-rebalance.md` | 新頁 `/rebalance`：PORTFOLIO BETA 半圓儀表＋目標 β 滑桿＋配置比例條＋偏離/再平衡建議面板（觸發閾值 ±%、上下限 band、精確達標金額）＋持倉編輯表；`lib/rebalance.ts` 純函式（投組 β＝加權平均、band、精確達標搬錢）＋vitest、`lib/rebalanceStore.ts` localStorage；**多標的每檔可調 β／手動填市值／零後端零請求零 LLM**；獨立於 opt9 可先做 | 無 | ⬜ 規格定稿 (2026-07-04) |
