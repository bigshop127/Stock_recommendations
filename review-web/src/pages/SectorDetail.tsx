...
export const SectorDetail: React.FC = () => {
  ...
  const fetchData = async (force = false) => {
    ...
    try {
      ...
      setError(err.message || '無法取得產業個股資料');
    } finally {
      setLoading(false);
    }
  };
  ...
  // Filter components in this sector
  const sectorStocks = useMemo<HeatmapStock[]>(() => {
    if (!data || !data.stocks) return [];
    return data.stocks.filter((s) => s.sector === sectorName);
  }, [data, sectorName]);
  ...
  // Treemap calculation for individual stocks
  const tiles = useMemo(() => {
    if (sectorStocks.length === 0) return [];

    const FLOOR = 0.05;
    const inputs: TreemapInput[] = sectorStocks
  ...
  if (error || !data) {
    return (
      <div className="h-[70vh] flex flex-col items-center justify-center text-center p-6 border border-zinc-800 bg-zinc-900/20 rounded-xl max-w-xl mx-auto my-12">
        ...
        <span className="text-sm text-red-400 font-semibold mb-2">載入產業資料失敗</span>
  ...
  if (sectorStocks.length === 0) {
    return (
      <div className="h-[70vh] flex flex-col items-center justify-center text-center p-6 border border-zinc-800 bg-zinc-900/20 rounded-xl max-w-xl mx-auto my-12">
        ...
        <span className="text-sm text-zinc-300 font-semibold mb-2">查無此產業：{sectorName}</span>
        <span className="text-xs text-zinc-500 font-mono mb-6">全市場熱力圖數據中無該產業成份股</span>
        <Link
          to={`/heatmap?period=${period}`}
          className="flex items-center gap-1.5 px-4 py-2 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 rounded-lg text-xs font-semibold transition"
        >
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="p-4 rounded-xl border border-zinc-800 bg-zinc-900/40 space-y-1">
          ...
          >
            {metrics.avgChangePct !== null && metrics.avgChangePct > 0 ? '+' : ''}
            {metrics.avgChangePct !== null ? metrics.avgChangePct.toFixed(2) : '0.00'} %
          </div>
        </div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 border border-zinc-800 bg-zinc-900/20 rounded-xl">
        <div className="text-[11px] text-zinc-500 leading-relaxed">
          * 區塊大小代表個股漲跌幅絕對值。歷史漲跌幅（週/月）採未還原收盤價計算，若期間經歷除權息可能影響計算精確度。
        </div>
