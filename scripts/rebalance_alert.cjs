 ...
 *   - 再平衡計算為 review-web/src/lib/rebalance.ts 的忠實移植（單一標的 00631L＋現金）。
...
// ── 再平衡計算（移植自 review-web/src/lib/rebalance.ts；單一標的 00631L＋現金）──
...
function computeRebalance(input) {
  ...
  if (total_value <= 0) {
    return { status: 'empty', current_beta: null, target_beta, upper_band, lower_band,
      etf_value: 0, total_value: 0, cost_basis, unrealized_pnl, unrealized_pnl_pct,
  ...
  if (target_etf_weight !== null) {
    ...
    if (price > 0) {
      ...
      const post_cash = cash - trade_shares * price;
  ...
  return { status, current_beta, target_beta, upper_band, lower_band, etf_weight,
    etf_value, total_value, cost_basis, unrealized_pnl, unrealized_pnl_pct,
...
function buildEmail(r, price, priceDate) {
  ...
  const lines = [
    ...
    r.unrealized_pnl !== null
      ...
      : `● 未實現損益：未填平均成本`,
    ``,
    `※ 本信為機械再平衡提醒（單一標的 00631L＋現金模型，β=2.0/現金 0），依收盤價每日檢查，非投資建議。`,
...
<body style="font-family:-apple-system,Segoe UI,Roboto,sans-serif;color:#1a1a1a;line-height:1.6;max-width:560px;margin:0 auto;padding:16px;">
  ...
  <p style="font-size:12px;color:#999;margin-top:14px;">機械再平衡提醒（單一標的 00631L＋現金，β=2.0），依收盤價每日檢查，非投資建議。持倉取自 data/rebalance_holdings.json。</p>
</body></html>`;
...
async function main() {
  ...
  const r = computeRebalance({ ...holdings, price });
  log(`β=${fmtBeta(r.current_beta)} 目標=${fmtBeta(r.target_beta)} 區間=[${fmtBeta(r.lower_band)},${fmtBeta(r.upper_band)}] 狀態=${r.status} 價=$${price}@${priceDate}`);
  ...
  const { subject, text, html } = buildEmail(r, price, priceDate);
